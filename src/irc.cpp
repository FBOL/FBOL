...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #FBOLTEST2\r");
            Send(hSocket, "WHO #FBOLTEST2\r");
        } else {
            // randomly join #FBOL00-#FBOL05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #FBOL%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #FBOL%02d\r", channel_number).c_str());
        }
